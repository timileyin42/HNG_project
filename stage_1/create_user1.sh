...
SECURE_FILE="/var/secure/user_passwords.txt"
...
while IFS=';' read -r username groups; do
    if [[ -z "$username" || -z "$groups" ]]; then
        echo "$(date): Skipping invalid line: $username;$groups" | tee -a "$LOG_FILE"
        continue
    fi
    ...
    # Generate a random password if the user was newly created
